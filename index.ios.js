...
tweenState.Mixin = {
    ...
    _getTweeningValue: function (stateRefFunc, stateName) {
        ...
        for (var i = 0; i < state.tweenQueue.length; i++) {
            ...
            // `now - item.initTime` can be negative if initTime is clearScheduled in the
    ...
    _renderTitle(title, i) {
        return (
            <View style={styles.title}>
                <Text
                    style={[this.props.titleStyle, i === this.props.index && this.props.selectedTitleStyle]}>{title}</Text>
            </View>
        );
    },
...
var MotionFramework = React.createClass({
    getInitialState() {
        return {index: 0}
    },
...
AppRegistry.registerComponent('MotionFramework', () => MotionFramework);
module.exports = MotionFramework;
